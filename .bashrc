...
then
	sudo pacman -S libttspico-utils -y
fi
...
alias hardmon="echo 'TEMPERATURE: ACPI | Skylake | CPU | Wifi (more info install lm_sensors, and run: sudo sensors-detect; sensors)'; cat /sys/class/thermal/thermal_zone*/temp; sudo lshw; cat /proc/cpuinfo; sudo lsusb; sudo libinput list-devices; sudo udevadm info --export-db"
...
debugdump(){
journalctl -p 3 -xb
sudo pacman -S apport-retrace python-problem-report python-apport -y
apropos apport
...
alias listrepo='cat /etc/pacman.conf | grep "\["'	
alias rmrepo='cat /etc/pacman.conf | grep "\[" && read -p "Name of repo: " REPO && pacman -Sl $REPO | grep "installed" | awk "{ print $2}" && sudo pacman -Rnsc $(pacman -Sl $REPO | grep \"\[installed\]\" | cut -f2 -d \" \")""'
...
searchinside() {
    ...
    #from https://raw.githubusercontent.com/alrra/dotfiles/master/src/shell/bash_functions
}
